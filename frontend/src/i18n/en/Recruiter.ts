// ============================================
// i18n/en/recruiter.ts — Recruiter page and recruiter mode content
// ============================================
export const recruiter = {
  "recruiter.seo.title": "Recruiter — Daren | Hednai",
  "recruiter.seo.desc": "Developer profile: marine background, React/Node/AI skills, completed projects.",
  "recruiter.back": "Back to site",
  "recruiter.title": "Daren — Full Stack Developer",
  "recruiter.tagline": "Merchant navy officer → developer | Maritime, Software, AI",
  "recruiter.contact": "Contact me",
  "recruiter.about.title": "Background",
  "recruiter.about.text": "After over 10 years as a merchant navy officer, I added a new chapter to my journey: software development. At La Cité College, I learned to design and build applications. Since then, I have been turning this foundation into real projects while continuing my path toward artificial intelligence. HEDNAI was born from this meeting between two worlds I know: the realities of the maritime field and the possibilities of technology.",
  "recruiter.skills.title": "Technical skills",
  "recruiter.skills.frontend": "Frontend",
  "recruiter.skills.backend": "Backend",
  "recruiter.skills.tools": "Tools and DevOps",
  "recruiter.skills.other": "Quality and best practices",
  "recruiter.skills.methods": "Methods and collaboration",
  "recruiter.skills.leadershipManagement": "Leadership and Managerial Skills (Transport Canada)",
  "recruiter.diff.title": "What sets me apart",
  "recruiter.diff.intro": "A career at the crossroads of maritime, software development and artificial intelligence.",
  "recruiter.diff.card1.title": "Maritime experience",
  "recruiter.diff.card1.text": "10+ years at sea. Rigor, precision, decision-making and adaptability in demanding environments.",
  "recruiter.diff.card2.title": "Software development",
  "recruiter.diff.card2.text": "Backend to frontend. Technical versatility, strong learning ability and understanding of the full application stack.",
  "recruiter.diff.card3.title": "Artificial intelligence",
  "recruiter.diff.card3.text": "Development + data + AI. Building skills to integrate AI into practical software solutions.",
  "recruiter.diff.card4.title": "Analytical mindset",
  "recruiter.diff.card4.text": "Understand before coding. Analyze the problem, its constraints and its users before choosing a technical solution.",
  "recruiter.cta": "Let's talk",
};

// ProfileSection — recruiter timeline
export const profile = {
  "profile.title": "Background",
  "profile.subtitle": "From sea to code — an atypical journey turned strength",
  "profile.photo.alt": "Daren — Full Stack Developer",
  "profile.photo.caption": "Capt. Daren MCS · Ottawa, ON",
  "profile.step1.period": "2011 — 2024",
  "profile.step1.title": "Merchant navy officer",
  "profile.step1.desc": "Over ten years aboard, between navigation, operations management, crew leadership and port environments. An experience that taught me to work with real constraints, make decisions and above all understand the maritime field.",
  "profile.step2.period": "May 2025 — Aug. 2026",
  "profile.step2.title": "Computer programming — La Cité College",
  "profile.step2.desc": "I started a new chapter without leaving the first behind: learning to build the tools I wish I had in my own professional environment. Software development, web, mobile, databases and exploring different technologies.",
  "profile.step3.period": "2025 — 2026",
  "profile.step3.title": "Development and personal projects",
  "profile.step3.desc": "Beyond coursework, I turn my ideas into real projects: applications, management tools, web solutions and AI experiments. It was through building that my idea of combining technology and maritime experience truly took shape.",
  "profile.step4.period": "Sep. 2026 — Apr. 2027",
  "profile.step4.title": "Artificial Intelligence Software Development — Algonquin College",
  "profile.step4.desc": "A new step to deepen software development and artificial intelligence, with the goal of turning more real problems into digital solutions.",
  "profile.step5.period": "2025 — Present",
  "profile.step5.title": "HEDNAI — From Sea to Code",
  "profile.step5.desc": "HEDNAI was born from the meeting of two worlds: the sea I know and the code I am learning to build. The goal is simple: develop digital solutions and AI tools that start from the realities of the maritime field, rather than technology for technology's sake.",
  "profile.next.title": "My Resume",
  "profile.next.text": "View my full background in PDF.",
};

// WhyHednai — recruiter variant ("What sets me apart")
export const whyRecruiter = {
  "why.title.recruiter": "What sets me apart",
  "why.subtitle.recruiter": "A career at the crossroads of maritime, software development and artificial intelligence.",
  "why.mission.title.recruiter": "Maritime experience",
  "why.mission.desc.recruiter": "10+ years at sea. Rigor, precision, decision-making and adaptability in demanding environments.",
  "why.vision.title.recruiter": "Software development",
  "why.vision.desc.recruiter": "Backend to frontend. Technical versatility, strong learning ability and understanding of the full application stack.",
  "why.values.title.recruiter": "Artificial intelligence",
  "why.values.desc.recruiter": "Development + data + AI. Building skills to integrate AI into practical software solutions.",
  "why.edge.title.recruiter": "Analytical mindset",
  "why.edge.desc.recruiter": "Understand before coding. Analyze the problem, its constraints and its users before choosing a technical solution.",
};

// CV toggle buttons in ProfileSection
export const profileCv = {
  "profile.showCv": "View my full resume",
  "profile.hideCv": "Close resume",
};